...
public class SilenceCommand extends AbstractChatCommand {

    public SilenceCommand() {
	super("silence", "Silence a player", "None", false, "silence", new String[0]);
    }
	...
	Bukkit.getScheduler().scheduleSyncDelayedTask(MCNSAChat.plugin, new Runnable() {
	    public void run() {
		if (PlayerManager.getPlayer(finalUUID, MCNSAChat.shortCode) != null && PlayerManager.getPlayer(finalUUID, MCNSAChat.shortCode).modes.get("S-MUTE")) {
