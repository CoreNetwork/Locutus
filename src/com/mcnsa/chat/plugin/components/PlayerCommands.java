...
//@ComponentInfo(friendlyName = "Player", description = "Player commands", permsSettingsPrefix = "player")
public class PlayerCommands {
    // @Command(command = "c", arguments = { "Channel" }, description =
    // "Move to a channel", permissions = { "move" }, playerOnly = true)
	...
	if (ChannelManager.getChannel(channel) != null) {
	    ...
	    if (!PermissionManager.checkPermission(chan.readPermission, player)) {
    ...
    // @Command(command = "listen", description = "Listen to a channel",
    // arguments = { "channel" }, permissions = {}, playerOnly = true)
	...
	if (targetChannel != null) {
	    ...
	    if (!PermissionManager.checkPermission(targetChannel.readPermission, player)) {
    ...
    // @Command(command = "csearch", description =
    // "Find what channel a player is in")
	...
	if ((target = PlayerManager.searchPlayer(player)) != null) {
	    MessageSender.send(target.getName() + " is in channel: " + target.channel, sender);
	} else {
	...
	for (ChatPlayer player : PlayerManager.players) {
	    String playerlistName = Colors.color(Colors.PlayerPrefix(player) + player.getName());
	    if (playerlistName.length() > 16)
		playerlistName = playerlistName.substring(0, 16);
	    Bukkit.getPlayer(player.getName()).setPlayerListName(playerlistName);
	}
	return true;
    }
